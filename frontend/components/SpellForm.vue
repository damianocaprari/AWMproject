<template>
  <v-row justify="center" v-if="alert">
    ...
    <v-col class="text-center">
      <v-btn class="accent onaccent--text" @click="returnToSpell">Back</v-btn>
    </v-col>
  </v-row>
  ...
  <v-form @submit.prevent="submit" v-else>
    ...
    <v-row>
      <v-col cols="12" sm="6">
        <v-row>
          ...
          <v-col cols="6">
            <v-text-field v-model="spell.casting_time_amount" type="number" label="Casting time amount"></v-text-field>
          </v-col>
          <v-col cols="6">
            <v-select v-model="spell.casting_time_unit" :items="castingTimeList" label="Casting time unit"></v-select>
          </v-col>
          ...
          <template v-if="spell.duration_type == 'CONCENTRATION' || spell.duration_type == 'SPECIAL' || spell.duration_type == 'TIME'">
            <v-col cols="12">
              <v-select v-model="spell.duration_type" :items="durationList" label="Duration type"></v-select>
            </v-col>
            <v-col cols="6">
        ...
        <v-row class="d-sm-none"> <!-- show only for screen <600 px -->
          <v-col cols="12">
            <v-textarea v-model="spell.description" label="Description" counter="8000" auto-grow ></v-textarea>
          </v-col>
        </v-row>

        <v-row>
          <v-col cols="12">
            <v-select v-model="spell.classes" :items="getCharacterclassesList()" label="Character classes" multiple></v-select>
          </v-col>
          <v-col cols="6">
      ...
      <v-col cols="6" sm="6" class="d-none d-sm-block"> <!-- show only for screen >=600 px -->
        <v-row>
          <v-col cols="12">
            <v-textarea v-model="spell.description" label="Description" counter="8000" auto-grow height="588" filled></v-textarea>
          </v-col>
        </v-row>
      </v-col>
    </v-row>
    ...
    <v-row justify="center">
      <v-col class="text-center">
        <v-btn type="submit" outlined color="accent" v-if="!alert">Save</v-btn>
